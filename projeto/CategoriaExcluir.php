...
echo("	</div>");
?>
	<div class="Retangulo_7"><div class="CADASTRO_DE_USUARIO">SELECIONE A CATEGORIA À EXCLUIR</div></div>
...
<?php
  ...
  $cmdsql="SELECT * FROM categoria ORDER BY idcategoria";
  printf("<br><br><p class=\"t1\"><form action='./CategoriaEsc.php' method='POST'>\n");
  $execcmd=mysqli_query($link,$cmdsql);
  printf("<span>Selecione a categoria</span>");
  printf("<select name='pick'>\n");
  while ( $REG=mysqli_fetch_array($execcmd) )
  {
    printf("<option value='$REG[idcategoria]'>$REG[idcategoria]-$REG[txnomecategoria]</option>\n");
  }
  printf("</p></select>\n");
