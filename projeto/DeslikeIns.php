...
while ( $tentativa )
{
	...
	if ($reg['verificacao'] == "")
	{
		...
		$cmd="INSERT INTO comentarioaux1 VALUES ('2', '$idusuario', '$_POST[cpcomentario]', '$idreceita')";
		mysqli_query($link,$cmd);
		$tentativa = 0;
	}
	if ($reg['verificacao'] == "1")
	{
		$proxpk=mysqli_fetch_array(mysqli_query($link,"SELECT max(dislike11)+1 as CMAX FROM comentario WHERE idcomentario = '$_POST[cpcomentario]'"));
		$num=( isset($proxpk['CMAX']) ) ? $proxpk['CMAX'] : 1;
		$cmd="UPDATE comentario SET dislike11 = '$num' WHERE idcomentario = '$_POST[cpcomentario]'";
		...
		mysqli_query($link,$cmd);
		$cmd="UPDATE comentarioaux1 SET verificacao = '2' WHERE receita_idreceita = '$idreceita' AND usuario_idusuario = '$idusuario' AND comentario_idcomentario = '$_POST[cpcomentario]'";
		mysqli_query($link,$cmd);
		$tentativa = 0;
	}
	else
