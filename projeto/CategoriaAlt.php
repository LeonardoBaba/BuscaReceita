...
echo("	</div>");
?>
	<div class="Retangulo_7"><div class="CADASTRO_DE_USUARIO">ALTERAR Categoria</div></div>
	...
	<form action='./CategoriaUpt.php' method='POST'>
	<p class="t1">
	<br><br>
<?php
global $link;
$link = mysqli_connect("localhost","root","","projetobusca") or die ("A conexão com o BD não deu certo");
$reg=mysqli_fetch_array(mysqli_query($link,"SELECT * FROM categoria WHERE idcategoria='$_POST[pick]'"));
echo("		<input type='hidden' name='idcategoria' value='$_POST[pick]'");	
echo("		<br>");
echo("		<span>NOME DA CATEGORIA</span>");
echo("		<input type='text' name='nomecategoria' size=80% maxlength=45 value=\"$reg[txnomecategoria]\">");
