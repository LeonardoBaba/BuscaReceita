...
echo("	<div class=\"Retangulo_7\"><div class=\"CADASTRO_DE_USUARIO\">LIVROS DE RECEITA</div></div>");
...
$sql = "SELECT * FROM livrodereceita WHERE idusuarioce = '$idusuario'";
$execsql = mysqli_query($link,$sql);
while ($reg = mysqli_fetch_array($execsql))
{
	printf("<div class=\"CADASTRO_DE_USUARIO\">$reg[txnomelivro]</div>");
	$sql1 = "SELECT * FROM livroreceitas WHERE idlivroce = '$reg[idlivro]'";
	$execsql1 = mysqli_query($link,$sql1);
	while ($reg1 = mysqli_fetch_array($execsql1))
	{
		$reg2 = $reg1['idreceitace'];
		$sql2 = "SELECT * FROM receita WHERE idreceita = '$reg2'";
		$execsql2 = mysqli_query($link,$sql2);
		while ($REGFINAL = mysqli_fetch_array($execsql2))
		{
		printf("<p class='k18'>$REGFINAL[txnomereceita]</p>");
		printf("<div class='R_11'><img src='imagem/$REGFINAL[tximagem]' class='square'></div>");
		printf("<p class='k18' style='font-size:20px;position:relative;left:220px;top:-230px'>Rendimento: $REGFINAL[txrendimento]</p>");
		printf("<p class='k18' style='font-size:20px;position:relative;left:220px;top:-230px'>Tempo de preparo: $REGFINAL[txtempopreparo]</p>");
		printf("<p class='k18' style='font-size:20px;position:relative;left:220px;top:-230px'>Modo de preparo: $REGFINAL[txmodopreparo]</p>");
		printf("\n");
		}
	}
}
