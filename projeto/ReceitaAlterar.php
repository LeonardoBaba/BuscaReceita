...
<div class="header">
	...
	<div class="Retangulo_7"><div class="CADASTRO_DE_USUARIO">SELECIONE A RECEITA</div></div>
...
<?php
	$idusuario = $_POST['usuariocp'];
  global $link;
  $link = mysqli_connect("localhost","root","","projetobusca") or die ("A conexão com o BD não deu certo");
  $cmdsql="SELECT * FROM receita WHERE idusuarioce='$idusuario'";
  printf("<br><br><p class=\"t1\"><form action='./ReceitaAux1.php' method='POST'>\n");
  $execcmd=mysqli_query($link,$cmdsql);
  printf("<span>SELECIONE A RECEITA PARA SER ALTERADA</span>");
  printf("<select name='pick'>\n");
  while ( $REG=mysqli_fetch_array($execcmd) )
  {
    printf("<option value='$REG[idreceita]'>$REG[idreceita]-$REG[txnomereceita]</option>\n");
  }
	printf("</p></select>\n");
  printf("<br><br>
