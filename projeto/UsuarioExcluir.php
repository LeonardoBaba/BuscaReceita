...
<div class="header">
	...
	<div class="Retangulo_7"><div class="CADASTRO_DE_USUARIO">EXCLUIR USUARIO</div></div>
...
<?php
  ...
  $cmdsql="SELECT * FROM usuario ORDER BY idusuario";
  printf("<br><br><p class=\"t1\"><form action='./UsuarioEsc.php' method='POST'>\n");
  $execcmd=mysqli_query($link,$cmdsql);
  printf("<select name='pick'>\n");
  while ( $REG=mysqli_fetch_array($execcmd) )
  {
    printf("<option value='$REG[idusuario]'>$REG[idusuario]-$REG[txnome]</option>\n");
  }
  printf("</select>\n");
